...
import { FaRobot, FaBrain, FaMicrochip, FaNetworkWired } from "react-icons/fa";
            ...
            <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                <div className={`mt-16 relative overflow-hidden bg-gradient-to-r ${currentTheme.gradient} text-white transition-all duration-700`}>
                    ...
                    <div className="container mx-auto px-4 py-8 relative z-10 flex flex-col items-center text-center">
                        <Link to="/ai-posts" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 p-2 rounded-full backdrop-blur-sm transition-all text-white/80 hover:text-white">
                        ...
                        <CardContent className="p-0">
                            ...
                            <div className="bg-gray-50/50 border-t border-gray-100 px-6 py-4 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <Button
                                        variant="ghost"
                                        className={`gap-2 ${post.likes > 0 ? "text-violet-600 bg-violet-50" : "text-gray-500 hover:text-gray-900"}`}
                                        onClick={handleLike}
                                    >
                                    ...
                                    <div className="bg-gray-50 rounded-xl p-4 mb-6 border border-gray-100 shadow-inner">
                                        <div className="flex gap-3">
                                            <Avatar className="h-8 w-8 mt-1">
                                                <AvatarFallback className="bg-violet-100 text-violet-600 font-bold">G</AvatarFallback>
                                            </Avatar>
                                            <div className="flex-1 relative">
                                                <Input
                                                    id="comment-input"
                                                    placeholder="Add a thought..."
                                                    className="min-h-[44px] py-2.5 pr-10 bg-white border-gray-200 rounded-lg focus-visible:ring-2 focus-visible:ring-violet-100 focus-visible:border-violet-300 transition-all font-light"
                                                    value={commentText}
                                                    onChange={(e) => setCommentText(e.target.value)}
                                                    onKeyDown={(e) => e.key === 'Enter' && handleComment()}
                                                />
                                                <Button size="icon" variant="ghost" className="absolute right-1.5 top-1.5 h-8 w-8 text-violet-600 hover:bg-violet-50 rounded-lg transition-colors" onClick={handleComment} disabled={!commentText.trim()}>
