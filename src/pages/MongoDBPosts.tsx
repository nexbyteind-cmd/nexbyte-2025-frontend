...
const CATEGORY_THEMES: Record<string, CategoryTheme> = {
    "MongoDB": {
        icon: SiMongodb,
        gradient: "from-green-900 via-emerald-900 to-green-900", // Green theme
...
const MongoDBPosts = () => {
    ...
    const selectedCategory = "MongoDB";
    ...
    const handleShare = async (id: string) => {
        ...
        const shareUrl = `${window.location.origin}/tech-posts/${id}`; // Reusing generic route for detail
    ...
    const currentTheme = CATEGORY_THEMES["MongoDB"];
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-4 mb-6 border border-gray-100">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <span className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide ${currentTheme.accentColor} text-white shadow-md`}>
                                    ...
                                    <div className="flex items-center border-r border-gray-200 pr-2 mr-2">
                                        <Popover>
                                            <PopoverTrigger asChild>
                                                <button className={`p-2 rounded-md transition-all ${date ? "text-green-600 bg-green-50" : "text-gray-500 hover:text-gray-900 hover:bg-white"}`}>
                                                    ...
                                                    <div className="w-full h-48 bg-gray-100 relative overflow-hidden">
                                                        ...
                                                        <div className="absolute top-3 left-3 z-20">
                                                            <span className="bg-white/90 backdrop-blur text-xs font-bold px-2 py-1 rounded-md shadow-sm border border-white/50 flex items-center gap-1.5 text-gray-800">
                                                                <CurrentIcon className={`w-3 h-3 ${currentTheme.textColor}`} />
                                                                {post.category}
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="flex flex-col flex-1 p-5">
                                                    <div className="mb-4 flex-1">
                                                        ...
                                                        {post.content && post.content.length > 150 && (
                                                            <button
                                                                ...
                                                                className="text-green-600 hover:text-green-700 text-xs font-bold mt-2 focus:outline-none hover:underline"
                                                            >
                                                    ...
                                                    <div className="flex items-center justify-between text-xs text-gray-400 border-t border-gray-50 pt-4 mt-auto">
                                                        ...
                                                        <div className="flex items-center gap-3 font-medium">
                                                            <span className="flex items-center gap-1 hover:text-green-600 transition-colors"><ThumbsUp className="w-3 h-3" /> {post.likes || 0}</span>
                                                            <span className="flex items-center gap-1 hover:text-green-600 transition-colors"><MessageCircle className="w-3 h-3" /> {post.comments?.length || 0}</span>
                                                        </div>
                                                    </div>

                                                    <div className="flex gap-2 pt-3 mt-3 border-t border-gray-50">
                                                        <Button variant="ghost" size="sm" className={`flex-1 h-8 text-xs rounded-lg transition-colors ${post.likes > 0 ? "text-green-600 bg-green-50 font-bold" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"}`} onClick={() => handleLike(post._id)}>
                                                        ...
                                                        <div className="mt-3 pt-3 border-t border-gray-100 animate-in fade-in slide-in-from-top-2">
                                                            <div className="relative group">
                                                                <input
                                                                    ...
                                                                    className="w-full pl-3 pr-9 py-2 text-xs bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:bg-white focus:ring-2 focus:ring-green-100 focus:border-green-300 transition-all"
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`absolute right-1 top-1 p-1 rounded-md transition-all ${commentValues[post._id] ? "text-green-600 hover:bg-green-50" : "text-gray-300 cursor-not-allowed"}`}
